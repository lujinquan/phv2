...
// halt($ids);
            ...
            if($type == 'unpaid'){ //欠缴列表里，点击的是order表
                ...
                foreach ($ids as $id) {
                    ...
                    if($row->getData('ptime') < $payStartTime || $row->getData('ptime') >= $payEndTime || $row->pay_way != 1 || $row->rent_order_status != 1){ //如果不是本月操作的实收订单，或者不是现金交的
                        ...
                        $trade_row = Db::name('weixin_order_trade')->where([['rent_order_id','eq',$row['rent_order_id']],['pay_dan_money','eq',$row['rent_order_paid']]])->field('out_trade_no')->order('trade_id desc')->find();
                        if(!empty($trade_row)){
    ...
    public function get_data($getData,$page = 1,$limit = 10)
    {
        //halt($page);
            ...
            $result = $data = [];
            $baseData = Db::name('rent_order')->alias('a')->join('house b','a.house_id = b.house_id','left')->join('tenant c','a.tenant_id = c.tenant_id','left')->join('ban d','b.ban_id = d.ban_id','left')->field($fields)->limit(100000)->where($where)->select(); //->limit(1000)
            ...
            foreach($baseData as $b){ //dump($b['rent_order_date']);dump($separate);halt('201906' > $separate);//halt($b);
                ...
                $data[$b['house_id']]['house_curr_month_send_wx_sms'] = $b['house_curr_month_send_wx_sms'];
                $data[$b['house_id']]['house_curr_month_send_tel_sms'] = $b['house_curr_month_send_tel_sms'];
                ...
                $data[$b['house_id']]['tenant_tel'] = $b['tenant_tel'];

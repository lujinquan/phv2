...
class Report extends Model
{
    ...
    public function getPaidRent()
    {
        ...
        $instid = input('param.inst_id/d',INST); //默认查询当前机构
        $useid = input('param.use_id/d',1); //默认查询住宅
        $curMonth = input('param.query_month',2020-06); //默认查询当前年月
        $month = str_replace('-','',$curMonth);
        $params = ParamModel::getCparams();
        $separate = substr($month,0,4).'00';
        $where = [];
        ...
        $houses = Db::name('house')->alias('a')->join('tenant c','a.tenant_id = c.tenant_id','left')->join('ban d','b.ban_id = d.ban_id','left')->field('a.house_id,a.house_number,c.tenant_name');
        foreach ($houses as $b) {
            $data[$b['house_id']]['number'] = $b['house_number'];
        }
        //dump($where);halt($baseData);

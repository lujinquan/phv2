...
class HouseReport extends Model
{
    ...
    public function get_by_damage($owner, $ins)
    {
        ...
        $below = Db::name('house')->alias('a')->join('ban b', 'a.ban_id = b.ban_id', 'left')->where($where)->where([['house_status', 'eq', 1]])->group('house_use_id')->column('house_use_id ,count(house_id) as house_ids');; //底部的户数统计
        for ($s = 1; $s < 4; $s++) {
            if (!isset($below[$s])) {
                $below[$s] = 0;
            }
    ...
    public function get_by_useNature($owner, $ins)
    {  //右侧的顺序是住宅，企业，机关
    ...
    public function get_by_institution($owner, $ins)
    {
        ...
        $where[] = ['ban_status', 'eq', 1];
        $where[] = ['ban_inst_id', 'in', config('inst_ids')[$ins]];
        $below = Db::name('house')->alias('a')->join('ban b', 'a.ban_id = b.ban_id', 'left')->where($where)->where([['ban_inst_id', 'in', config('inst_ids')[$ins]]])->where([['house_status', 'eq', 1]])->group('house_use_id')->column('house_use_id ,count(house_id) as house_ids');; //底部的户数统计
        for ($s = 1; $s < 4; $s++) {
            if (!isset($below[$s])) {
                $below[$s] = 0;
            }
    ...
    public function get_by_value($owner, $ins)
    {  //右侧的顺序是住宅，企业，机关

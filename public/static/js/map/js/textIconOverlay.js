...
(function () {
    ...
    */
    TextIconOverlay.prototype._updateText = function(){
        if (this._domElement) {
            this._domElement.innerHTML = '<p class="j-cover-list j-name" title="'+this._text.name+'">'+this._text.name+'</p>'+'<p class="j-cover-list j-value">'+this._text.value+'</p>'+'<p class="j-cover-list j-household">'+this._text.household+'</p>';
        }
    };
    ...
    TextIconOverlay.prototype._buildCssText = function(style) {
        ...
        if (size instanceof BMap.Size){
            if (anchor instanceof BMap.Size) {
                ...
                csstext.push('height:' + size.height + 'px; line-height:' + 15 + 'px;');
